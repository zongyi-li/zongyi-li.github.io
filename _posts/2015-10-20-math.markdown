...
where $$\delta_x$$ is the delta measure on $$\R^d$$ centered at $$x$$. 
Note that $$G$$ will depend on the coefficient $$a$$ thus we will henceforth denote it as \(G_a\). The solution to \eqref{eq:generalpde} can then be represented as
